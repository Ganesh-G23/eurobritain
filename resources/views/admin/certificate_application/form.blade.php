        ...
        <div class="card">
            ...
            <div class="card-body">
                <form id="ajax-form" method="POST"
                    action="{{ $isEdit ? url('admin/certificate-application/update/' . $details->id) : url('admin/certificate-application/save_application') }}">
                    @csrf
...
@section('scripts')
    <script>
        ...
        function buildAddressBlock(addrIndex, addressVal = '', shifts = [{from:'',to:''}]) {
            let shiftsHtml = '';
            (shifts || [{from:'',to:''}]).forEach(function(shift, sIdx) {
        ...
        function reindexAddressBlocks() {
            $('#address-shift-container .address-block').each(function(addrIdx) {
                ...
                $(this).find('[name^="address_shift_details"]').each(function() {
                    ...
                    if (!name) return;
                    const updated = name.replace(/address_shift_details\[\d+\]/, 'address_shift_details[' + addrIdx + ']');
        ...
        function addAddressBlock(addressVal = '', shifts = null) {
            ...
            $('#address-shift-container').append(buildAddressBlock(addrIndex, addressVal, shifts || [{from:'',to:''}]));
        }

        if (initialAddressShifts && initialAddressShifts.length) {
            initialAddressShifts.forEach(function(item) {
                addAddressBlock(item.address || '', item.shifts || [{from:'',to:''}]);
            });
        } else {
            addAddressBlock();
        }
