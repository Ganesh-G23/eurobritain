    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card">
            <div class="card-header d-flex flex-wrap justify-content-between align-items-center gap-2">
                ...
                <a href="{{ url('admin/certificate-application/list') }}" class="btn btn-label-secondary btn-sm">Application List</a>
            </div>
            <div class="card-body">
                <div class="row mb-4">
                    <div class="col-md-6 mb-3">
                        <label class="form-label" for="associate_id">Associate <span class="text-danger">*</span></label>
                        <select class="form-select" id="associate_id">
                            <option value="">Select Associate</option>
                            @foreach ($associates as $associate)
                                <option value="{{ $associate->id }}">{{ $associate->company_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6 mb-3">
                        <label class="form-label" for="client_id">Company Name <span class="text-danger">*</span></label>
                        <select class="form-select" id="client_id" disabled>
                ...
                <div id="client-forms-area">
                    @include('admin.certificate_application.partials.new_form')
                    @include('admin.certificate_application.partials.old_form')
                </div>
            </div>
        </div>
    </div>
...
@section('scripts')
    <script>
        ...
        function loadCertificateTypesFor(prefix) {
            const associateId = $('#associate_id').val();
            ...
            if (!associateId || !type || !category) {
                return;
            }

            $dropdown.prop('disabled', true).empty().append('<option value="">Loading...</option>');

            $.get('{{ url('admin/certificate-application/certificate-types') }}', {
        ...
        $('#associate_id').on('change', function() {
            ...
            resetClientForms();
            $client.prop('disabled', true).empty().append('<option value="">Select company</option>');
            ...
            $.get('{{ url('admin/certificate-application/clients') }}', {
                associate_id: associateId
            }, function(res) {
                $client.empty().append('<option value="">Select company</option>');
        ...
        $('#client_id').on('change', function() {
            ...
            $.get('{{ url('admin/certificate-application/check_client') }}', {
                client_id: clientId
            }, function(res) {
