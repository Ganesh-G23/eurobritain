...
<head>
    ...
    <title>Select Student - EliteGrade</title>
    ...
    <div class="page-wrap">
        <div class="page-header">
            <div class="page-header-inner">
                ...
                @endphp
                <div class="brand"><img src="{{ url('public/admin_theme/assets/img/logo.png') }}" alt="EliteGrade"
                ...
                <div class="header-actions dropdown">
                    <a class="d-inline-flex align-items-center justify-content-center rounded-circle"
                        href="javascript:void(0)" id="miniUserMenu" data-bs-toggle="dropdown" aria-expanded="false"
                        style="width:36px;height:36px;background:#e5e7eb;color:#111827;text-decoration:none;">
        ...
        <div class="headerless-container">
            <div class="card">
                <div class="card-body">
                    @php $parent = session('portal_user') ?? []; @endphp
                    <div class="greet">Hello {{ $parent['name'] ?? 'Parent' }}</div>
                    <div class="page-title">Select Your Student</div>
                    <div class="subtext">Choose a student to access their portal.</div>

                    @if (($students ?? collect())->count() === 0)
                        <div class="card-teacher">
                            <div class="card-title">No students linked yet</div>
                            <div class="card-sub">Please contact your institute or the respective teacher to link
                                students.</div>
                        </div>
                    @else
                            ...
                            <div class="card-teacher">
                                <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap">
                                    <div class="flex-grow-1">
                                        <div class="card-title">{{ $s->student_name }}</div>
                                        @if ($s->student_email)
                                            <div class="card-sub">Email: {{ $s->student_email }}</div>
                                        @endif
                                        @if ($s->student_phone)
                                            <div class="card-sub">Phone: {{ $s->student_phone }}</div>
                                        @endif
                                        @if ($s->batch_name)
                                            <div class="card-sub">Batch: {{ $s->batch_name }}</div>
                                        @endif
                                        @if ($s->classroom_name)
                                            <div class="card-sub">Classroom: {{ $s->classroom_name }}</div>
                                        @endif
                                    </div>
                                    <div>
                                        <form method="post" action="{{ url('user/select-student/access') }}">
                                            @csrf
